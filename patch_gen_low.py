...
# 创建输出目录
os.makedirs('low/uncertain_data', exist_ok=True)
os.makedirs('low/uncertain_label', exist_ok=True)

for i in range(len(imglist)):
    try:
        img_path = 'low/train_data/images/' + imglist[i]
        ...
        for nk in range(len(c_list[0])):
            # 保存图像patch
            skimage.io.imsave('low/uncertain_data/' + imglist[i].split('.jpg')[0] + '_' + str(nk) + '.png',
                              c_list[0][nk])
