...
class SHHA_unsup(Dataset):
    def __init__(self, data_root, img_list, transform=None, train=False, patch=False, flip=False):
        self.root_path = data_root

        full_list = os.listdir(self.root_path + 'train_data/images/')
        sup_list = np.loadtxt(img_list, dtype = str)
        un_list = []
        for ele in full_list:
            if ele not in sup_list:
                un_list.append( ele )   

        if train:
            self.img_list = un_list
        else:
