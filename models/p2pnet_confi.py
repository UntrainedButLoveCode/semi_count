...
# the network frmawork of the confidence branch
...
#         self.output = nn.Conv2d(16, 1, kernel_size=3, stride = 2, padding=1)
...
class confi_head(nn.Module):
    def __init__(self, num_features_in):
        ...
        self.conv1 = nn.Conv2d(num_features_in, 128, kernel_size=3,stride = 2, padding=1)
        self.act1 = nn.ReLU()
        # self.upsample1 = nn.Upsample(scale_factor=0.25, mode='bilinear')
        ...
        self.output = nn.Conv2d(64, 1, kernel_size=3, stride = 2, padding=1)
...
class P2PNet_confi_1(nn.Module):
    def __init__(self, backbone, row=2, line=2):
        ...
        self.confidence = confi_head(num_features_in=256)
    ...
    def forward(self, samples: NestedTensor):
        ...
        confidence_output = self.confidence(features_fpn[1])
...
def build_confi(args, training):
    # treats persons as a single class
    num_classes = 1

    backbone = build_backbone(args)
    model = P2PNet_confi_1(backbone, args.row, args.line)
    if not training: 
        return model
